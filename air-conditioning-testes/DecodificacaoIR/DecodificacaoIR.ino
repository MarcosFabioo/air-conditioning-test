...
const uint8_t kTimeout = 15; // posso redefinir para não engolir repetições
const uint16_t kMinUnknownSize = 12;x
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    Serial.print(resultToHumanReadableBasic(&results)); // PROTOCOLO
    dumpACInfo(&results);  // Display any extra A/C info if we have it. // CODE E MSG DESC
    yield();  // Feed the WDT as the text output can take a while to print.
    // Display the library version the message was captured with.
    Serial.print("Library   : v");
    Serial.println(_IRREMOTEESP8266_VERSION_); // LIBRARY
    Serial.println();
    // Output RAW timing info of the result.
    Serial.println(resultToTimingInfo(&results));
    yield();  // Feed the WDT (again)
    // Output the results as source code
    Serial.println("esse aq");
